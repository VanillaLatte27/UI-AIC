        ...
        createApp({
            setup() {
                ...
                async function changeTarget() {
                    ...
                    try {
                        const response = await fetch("http://localhost:5000/set_target", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: targetName.value.trim() }) });
